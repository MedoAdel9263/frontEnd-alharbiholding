...
<div className='keen-slider__slide !overflow-visible'>
    ...
    <img src={image} className='rounded-[5px] object-cover h-auto  w-[20rem] lg:max-h-[22rem]'/>
    </div>
    <div data-aos='fade-left' className='flex-1 mt-[3rem] lg:mt-0'>
        <h3 className='text-md font-bold leading-[1.25] !text-dynamic text-primary mb-[1.5rem]'>{Constants.ISPRODACTION ? item.Title : item.Title}</h3>
        <div dangerouslySetInnerHTML={{ __html: Constants.ISPRODACTION ? item.Description : item.Description }} className='mt-8 text-[#848383]'>
        </div>
    </div>
        </div>
    </div>
