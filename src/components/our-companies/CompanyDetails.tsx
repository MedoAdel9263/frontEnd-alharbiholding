                        ...
                        <div className='h-full rounded-[5px] bg-accent-100 p-6 transition-transform duration-350 hover:[transform:translateY(-.5rem)] dark:bg-accent-700 sm:p-[50px]'>
                            <div className='flex flex-col gap-12 xl:flex-row md:gap-5 md:md-[40%] justify-center items-center'>
                                <div className='relative  flex-none w-[25%] text-center'>
                                    <img src={Constants.ISPRODACTION ? `${Constants.HOSTURL}${item?.Photo}` : item?.Image.replace(new RegExp('/company-details/'), '/')} className='rounded-[5px] object-cover w-[12rem] m-auto' />
                                </div>
                                <div className='flex-1'>
                                    ...
                                    <a href={item?.Url} target='_blank' className='flex items-center gap-2.5 mt-8 '>
                                        <span className="flex-none text-sm py-2 px-10 rounded-[50px] font-xl text-primary bg-primary text-white hover:text-primary hover:bg-white border border-primary"> {item?.Url}</span>
                                    </a>
                                </div>
